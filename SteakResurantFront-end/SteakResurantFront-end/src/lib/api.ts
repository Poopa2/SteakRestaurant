...
export async function ensureSessionToken(): Promise<{ token: string; orderId: number }> {
  const urlToken = new URLSearchParams(location.search).get('token');
  if (urlToken) {
    const o = await fetch(`/api/sessions/${encodeURIComponent(urlToken)}`).then(r => r.json());
...
export const api = {
  ...
  orderItemsByOrder: async (orderId: number): Promise<OrderItem[]> =>
    fetch(`/api/orderitems/byorder/${orderId}`).then(r => r.json()),

  addItem: async (payload: { orderId: number; productId: number; quantity: number; price: number }) =>
